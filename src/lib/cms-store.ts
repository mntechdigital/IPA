...
export async function getCmsMeta(): Promise<{ version: string; lastUpdated: string }> {
  const meta = await db.cmsMeta.findUnique({ where: { id: 1 } });
...
export async function getHomePage(): Promise<HomePageData> {
  const row = await db.homePageData.findUnique({ where: { id: 1 } });
  return (row?.data ?? {}) as unknown as HomePageData;
}

export async function updateHomePage(data: HomePageData): Promise<void> {
  await db.homePageData.upsert({ where: { id: 1 }, update: { data: data as object }, create: { id: 1, data: data as object } });
}

export async function getAboutPage(): Promise<AboutPageData> {
  const row = await db.aboutPageData.findUnique({ where: { id: 1 } });
  return (row?.data ?? {}) as unknown as AboutPageData;
}

export async function updateAboutPage(data: AboutPageData): Promise<void> {
  await db.aboutPageData.upsert({ where: { id: 1 }, update: { data: data as object }, create: { id: 1, data: data as object } });
}

export async function getContactPage(): Promise<ContactPageData | null> {
  const row = await db.contactPageData.findUnique({ where: { id: 1 } });
...
export async function updateContactPage(data: ContactPageData): Promise<void> {
  await db.contactPageData.upsert({ where: { id: 1 }, update: { data: data as object }, create: { id: 1, data: data as object } });
}
...
export async function updateSiteSettings(data: SiteSettings): Promise<void> {
  await db.siteSettings.upsert({ where: { id: 1 }, update: { data: data as object }, create: { id: 1, data: data as object } });
}

export async function getMonitoring(): Promise<MonitoringTelemetry> {
  const row = await db.monitoringTelemetry.findUnique({ where: { id: 1 } });
...
export async function updateMonitoring(data: MonitoringTelemetry): Promise<void> {
  await db.monitoringTelemetry.upsert({ where: { id: 1 }, update: { data: data as object }, create: { id: 1, data: data as object } });
}
...
export async function createInquiry(data: Omit<InquirySubmission, 'id' | 'date'>): Promise<InquirySubmission> {
  ...
  const row = await db.inquiry.create({
    data: { id, date, name: data.name, organization: data.organization, email: data.email, topic: data.topic, message: data.message, requestedDataset: data.requestedDataset ?? null, status: data.status ?? 'new', notes: null },
  });
...
export async function resetCmsData(): Promise<void> {
  ...
  await db.cmsMeta.create({ data: { id: 1, version: s.version ?? '1.0.0', lastUpdated: new Date(s.lastUpdated ?? Date.now()) } });
  await db.siteSettings.create({ data: { id: 1, data: s.settings as object } });
  await db.homePageData.create({ data: { id: 1, data: s.homePage as object } });
  await db.aboutPageData.create({ data: { id: 1, data: s.aboutPage as object } });
  if (s.contactPage) await db.contactPageData.create({ data: { id: 1, data: s.contactPage as object } });
  await db.monitoringTelemetry.create({ data: { id: 1, data: s.monitoring as object } });
  for (const beat of Object.values(s.researchBeats)) {
    await db.researchBeat.create({ data: { id: beat.id, beatNumber: beat.beatNumber, category: beat.category, name: beat.name, tagline: beat.tagline, description: beat.description, image: beat.image, outputsCount: beat.outputsCount, timeframe: beat.timeframe, status: beat.status, leadFellows: (beat.leadFellows ?? []) as never, metrics: (beat.metrics ?? []) as never, overview: (beat.overview ?? []) as never, keyQuestions: (beat.keyQuestions ?? []) as never, methodologyDetails: (beat.methodologyDetails ?? []) as never, caseStudies: (beat.caseStudies ?? []) as never, publications: (beat.publications ?? []) as never, summary: beat.summary, methodology: beat.methodology, primaryMethodologies: (beat.primaryMethodologies ?? []) as never, imageTitle: beat.imageTitle, imageSubtitle: beat.imageSubtitle, researchNarrative: beat.researchNarrative, viewCount: beat.viewCount ?? 0 } });
  }
...
export async function importCmsState(payload: Partial<CmsState>): Promise<void> {
  const s = { ...INITIAL_CMS_STATE, ...payload, settings: { ...INITIAL_CMS_STATE.settings, ...(payload.settings || {}) }, homePage: { ...INITIAL_CMS_STATE.homePage, ...(payload.homePage || {}) }, aboutPage: { ...INITIAL_CMS_STATE.aboutPage, ...(payload.aboutPage || {}) }, contactPage: payload.contactPage || INITIAL_CMS_STATE.contactPage, researchBeats: payload.researchBeats ?? INITIAL_CMS_STATE.researchBeats, team: payload.team ?? INITIAL_CMS_STATE.team, publications: payload.publications ?? INITIAL_CMS_STATE.publications, monitoring: payload.monitoring ?? INITIAL_CMS_STATE.monitoring, inquiries: payload.inquiries ?? INITIAL_CMS_STATE.inquiries, activityLogs: payload.activityLogs ?? [] };
  await resetCmsData();
  await db.cmsMeta.update({ where: { id: 1 }, data: { version: s.version ?? '1.0.0', lastUpdated: new Date(s.lastUpdated ?? Date.now()) } });
  await db.siteSettings.update({ where: { id: 1 }, data: { data: s.settings as object } });
  await db.homePageData.update({ where: { id: 1 }, data: { data: s.homePage as object } });
  await db.aboutPageData.update({ where: { id: 1 }, data: { data: s.aboutPage as object } });
  if (s.contactPage) { await db.contactPageData.upsert({ where: { id: 1 }, update: { data: s.contactPage as object }, create: { id: 1, data: s.contactPage as object } }); }
  await db.monitoringTelemetry.update({ where: { id: 1 }, data: { data: s.monitoring as object } });
  for (const beat of Object.values(s.researchBeats)) {
    await db.researchBeat.upsert({ where: { id: beat.id }, update: { beatNumber: beat.beatNumber, category: beat.category, name: beat.name, tagline: beat.tagline, description: beat.description, image: beat.image, outputsCount: beat.outputsCount, timeframe: beat.timeframe, status: beat.status, leadFellows: (beat.leadFellows ?? []) as never, metrics: (beat.metrics ?? []) as never, overview: (beat.overview ?? []) as never, keyQuestions: (beat.keyQuestions ?? []) as never, methodologyDetails: (beat.methodologyDetails ?? []) as never, caseStudies: (beat.caseStudies ?? []) as never, publications: (beat.publications ?? []) as never, summary: beat.summary, methodology: beat.methodology, primaryMethodologies: (beat.primaryMethodologies ?? []) as never, imageTitle: beat.imageTitle, imageSubtitle: beat.imageSubtitle, researchNarrative: beat.researchNarrative, viewCount: beat.viewCount ?? 0 }, create: { id: beat.id, beatNumber: beat.beatNumber, category: beat.category, name: beat.name, tagline: beat.tagline, description: beat.description, image: beat.image, outputsCount: beat.outputsCount, timeframe: beat.timeframe, status: beat.status, leadFellows: (beat.leadFellows ?? []) as never, metrics: (beat.metrics ?? []) as never, overview: (beat.overview ?? []) as never, keyQuestions: (beat.keyQuestions ?? []) as never, methodologyDetails: (beat.methodologyDetails ?? []) as never, caseStudies: (beat.caseStudies ?? []) as never, publications: (beat.publications ?? []) as never, summary: beat.summary, methodology: beat.methodology, primaryMethodologies: (beat.primaryMethodologies ?? []) as never, imageTitle: beat.imageTitle, imageSubtitle: beat.imageSubtitle, researchNarrative: beat.researchNarrative, viewCount: beat.viewCount ?? 0 } });
  }
